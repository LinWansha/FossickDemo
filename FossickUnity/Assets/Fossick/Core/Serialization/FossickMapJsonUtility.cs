...
namespace Fossick.Core.Serialization
{
    public static class FossickMapJsonUtility
    {
        ...
        public static FossickMapProjectConfig ProjectFromJson(string json)
        {
            if (!string.IsNullOrEmpty(json)
                && json.Contains("\"fragmentLibrary\"")
                && json.Contains("\"generationRules\"")
                && json.Contains("\"mapDefinition\""))
            {
                return NormalizeProject(JsonUtility.FromJson<FossickMapProjectConfig>(json));
            }

            var legacy = FromJson(json);
            return NormalizeProject(FossickMapProjectConfig.FromRuntimeConfig(legacy, 12345));
        }
    ...
    public static class FossickMapProjectFileService
    {
        ...
        {
#if UNITY_EDITOR
            return Path.Combine(Application.dataPath, "Fossick/MapStudio/Maps");
#else
        ...
        {
            return Path.Combine(Application.persistentDataPath, RelativeMapsFolder);
        }

        public static string GetBundledPlayerMapsFolder()
        {
            return Path.Combine(Application.streamingAssetsPath, RelativeMapsFolder);
        }
